...
router.get('/getExpenses', (req, res) => {
    Expense.find((err, expenses) => {
        if (err) {
            return res.status(400).json({ msg: "Couldn't find any expenses. Have you tried adding one?" });
        }
...
router.post('/createExpense', (req, res) => {
    ...
    // Input validation
    if (!category || !price || typeof(category) != "string" || typeof(price) != "number") {
    ...
    expense.save((err) => {
        if (err) {
            return res.status(400).json({ msg: "There was a problem creating the expense." });
        }
    });
